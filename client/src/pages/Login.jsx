  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      <form action="">
        ...
        <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigoto -indigo-900 text-black font-medium'>{state}</button>

      </form>

      {state === 'Sign Up' ? (<p className='text-gray-400 text-center text-xs mt-4'>Already have an Account? 
        <span onClick={()=>setState('Login')} className='text-indigo-500 cursor-pointer'>Login here</span></p>):(<p className='text-gray-400 text-center text-xs mt-4'>Don't have an account? 
        <span onClick={()=>setState('Sign Up')} className='text-indigo-500 cursor-pointer'>Sign up here</span></p>)}
